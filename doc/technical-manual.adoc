...
Docker is a containerization platform that you can use to compose an application
made from several services. You can use docker in development to get started
quickly, or read the following sections to install all dependencies manually.

First you need to pull osis and its submodule osis-common:

    $ git pull git@github.com:uclouvain/osis.git
    $ cd osis
    $ git submodule init
    $ git submodule update

Next, you have to define a few environment variables. The default ones should work
with docker so simply copy the example file. They are used to setup the postgres
and rabbitmq user, password and host for your local environment.

    $ cp .env.example .env

Then all you have to do is to use docker compose to build the necessary containers and run it.

    $ docker-compose build
    $ docker-compose up

Once this is done, you can just seed the database using the information from <<seed-data>>.

You can stop the whole application with:

    $ docker-compose stop

Or stop a single container with:

    $ docker-compose backoffice stop

Once the containers are started, for each command you want to run inside an container, you have to
do something like:

    $ docker run container_name your_command

For instance, after the first docker start, you need to run the migrations like this:

    $ docker run backoffice python manage.py migrate # this will execute the pending migrations
