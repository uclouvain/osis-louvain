...
def get_next_link_year(old_gey: 'GroupElementYear', copy_to_year: 'AcademicYear') -> Optional['GroupElementYear']:
    msg = None
    old_parent = old_gey.parent
    old_branch = old_gey.child_branch
    old_leaf = old_gey.child_leaf

    model_fields = {field.name
                    for field in GroupElementYear._meta._get_fields(reverse=False, include_hidden=True)}
    fields_to_update = model_fields - {
        'parent',
        'child_branch',
        'child_leaf'
    ...
    defaults = {field: getattr(old_gey, field) for field in fields_to_update}
    ...
    try:
        ...
        msg = 'Link between {} and {} for {} has not been created'.format(
            old_parent,
