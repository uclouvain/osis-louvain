...
class ReddotEducationGroupAutomaticPostponement(AutomaticPostponement):
    ...
    def _postpone_cms(old_egy, new_egy):
        TranslatedText.objects.filter(entity=entity_name.OFFER_YEAR, cms_offers=new_egy).delete()

        for text in TranslatedText.objects.filter(entity=entity_name.OFFER_YEAR, cms_offers=old_egy):
