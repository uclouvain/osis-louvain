...
class TestAdministrativeDataForm(TestCase):
    ...
    def test_get_form_kwargs(self):
        ...
        acs = SessionExamCalendar.objects.filter(
            number_session=1,
            academic_calendar__data_year=self.education_group_year.academic_year
        ).values_list('academic_calendar_id', flat=True)

        self.assertEqual(
            list(result.get('list_offer_year_calendar')),
            list(OfferYearCalendar.objects.filter(
