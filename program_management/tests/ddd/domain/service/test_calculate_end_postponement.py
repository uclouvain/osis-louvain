...
class TestCalculateEndPostponementYear(MockPatcherMixin, TestCase):
    ...
    def test_when_end_year_is_none(self):
        ...
        with override_settings(YEAR_LIMIT_EDG_MODIFICATION=self.current_year - 1):
    ...
    def test_when_academic_year_is_lte_year_limit_edg_modification_settings(self):
        year_limit_edg_modification = 2019
        training = TrainingFactory(entity_identity__year=year_limit_edg_modification, end_year=None)
        fake_training_repository = get_fake_training_repository([training])

        with override_settings(YEAR_LIMIT_EDG_MODIFICATION=2019):
            result = CalculateEndPostponement.calculate_end_postponement_year_training(
                identity=training.entity_id,
                repository=fake_training_repository,
            )
            self.assertEqual(result, year_limit_edg_modification)
