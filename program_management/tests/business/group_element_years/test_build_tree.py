...
class TestBuildTree(TestCase):
        ...
        test_cases = [
            {'name': 'with tab',
             ...
             'correct_url': reverse(
                 ...
                 args=[self.parent.pk, self.parent.pk]) + "?group_to_parent=0&tab_to_show=show_identification"
             },
            {'name': 'without tab',
             'node': EducationGroupHierarchy(self.parent),
    ...
    def test_contains_luy_service_mobility(self):
        ...
        eluy = ExternalLearningUnitYearFactory(
            ...
            mobility=True,
            co_graduation=False,
            learning_unit_year__learning_container_year__requirement_entity=self.MATH.entity,
            learning_unit_year__learning_container_year__allocation_entity=self.URBA.entity
        )
    ...
    def test_contains_luy_borrowed_mobility(self):
        ...
        eluy = ExternalLearningUnitYearFactory(
            ...
            mobility=True,
            co_graduation=False,
            learning_unit_year__learning_container_year__requirement_entity=self.BARC.entity,
            learning_unit_year__learning_container_year__allocation_entity=self.BARC.entity
        )
        luy = LearningUnitYear.objects.get(externallearningunityear=eluy)
        GroupElementYearFactory(
            parent=self.group_element_year_2.child_branch,
...
class TestFetchGroupElementsBehindHierarchy(TestCase):
    ...
    def test_with_one_root_id(self):
        ...
        result = program_management.business.group_element_years.group_element_year_tree.\
            fetch_all_group_elements_in_tree(self.root, queryset)
        expected_result = {
    ...
    def test_when_queryset_is_not_from_group_element_year_model(self):
        ...
        with self.assertRaises(AttributeError):
            program_management.business.group_element_years.group_element_year_tree.\
                fetch_all_group_elements_in_tree(self.root, wrong_queryset_model)
