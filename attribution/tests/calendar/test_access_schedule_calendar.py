...
class TestAccessScheduleCalendarEnsureConsistencyUntilNPlus6(TestCase):
    @classmethod
    ...
    def test_ensure_consistency_until_n_plus_6_assert_default_value(self):
        ...
        qs = AcademicCalendar.objects.filter(reference=AcademicCalendarTypes.ACCESS_SCHEDULE_CALENDAR.name)
        ...
        self.assertDictEqual(
            model_to_dict(qs.first(), fields=('title', 'reference', 'data_year', 'start_date', 'end_date')),
            {
                ...
                "end_date": datetime.date(self.current_academic_year.year + 1, 9, 14),
            }
        )
