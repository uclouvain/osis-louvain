...
class TestCheckIssueFields(SimpleTestCase, MockPatcherMixin):
    def setUp(self):
        ...
        self.command = GetTrainingIssueFieldsOnWarningCommand(
            acronym=self.training.entity_id.acronym,
            year=self.training.entity_id.year
        )

        self.mock_service(
            "education_group.ddd.domain.service.fields_with_alert_when_issue.get_for_training",
            ["management_entity", "administration_entity"]
        )
    ...
    def test_should_not_return_exception_when_no_empty_fields(self, mock_get_entity_active):
        result = check_training_issue_fields_on_warning(self.command)
    ...
    def test_should_raise_exception_when_empty_fields(self, mock_get_entity_active):
        with self.assertRaises(exception.TrainingAlertFieldException):
            check_training_issue_fields_on_warning(self.command)
