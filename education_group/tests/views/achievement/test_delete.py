...
class TestDeleteAchievement(TestCase, TestAchievementMixin):
    @classmethod
    ...
    def test_should_remove_achievement(self):
        self.client.post(self.url, data=self.generate_post_data())

        with self.assertRaises(EducationGroupAchievement.DoesNotExist):
            EducationGroupAchievement.objects.get(id=self.achievement.id)

    def test_should_postpone_remove_when_postpone_parameter_set(self):
...
class TestDeleteDetailedAchievement(TestCase, TestAchievementMixin):
    @classmethod
    def setUpTestData(cls):
        cls.person = SuperUserPersonFactory()
    ...
    def setUp(self) -> None:
        ...
        self.url = reverse(
            "training_detailed_achievement_delete",
            args=[
                self.education_group_year.academic_year.year,
                self.education_group_year.partial_acronym,
                self.achievement.id,
    ...
    def test_should_update_detailed_achievement(self):
        ...
        with self.assertRaises(EducationGroupDetailedAchievement.DoesNotExist):
            EducationGroupDetailedAchievement.objects.get(id=self.detailed_achievement.id)

    def test_should_postpone_when_postpone_set(self):
        self.client.post(self.url, data=self.generate_post_data(to_postpone=True))
