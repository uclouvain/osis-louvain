...
class TestGroupYearVersionManager(TestCase):
    ...
    def test_with_education_group_version(self):
        self.group_yr_2 = GroupYearFactory(
            group=self.group,
            academic_year=self.academic_year_2019
        )
        self.education_grp_version = EducationGroupVersionFactory(root_group=self.group_yr_2)
        self.assertListEqual(list(GroupYear.objects_version.all()), [self.group_yr_2])
