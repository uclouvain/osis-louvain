...
from education_group.api.views.training import TrainingList, TrainingDetail, TrainingTitle, TrainingVersions
...
urlpatterns = [
    ...
    url(r'^trainings/(?P<year>[\d]{4})/(?P<acronym>[\w]+(?:[/| ]?[a-zA-Z]{1,2})?)/', include([
        ...
        url(r'^versions$', TrainingVersions.as_view(), name=TrainingVersions.name),
    ])),
